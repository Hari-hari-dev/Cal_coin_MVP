...
// Put your actual program ID here.
declare_id!("EVr3FKuQSjozzrmzkRfbxidMiT6zwBHvf6KJYcuMqHBK");
...
pub mod daily_facescan {
    ...
    /// (1) Initialize a new Airdrop + SPL Mint.
    pub fn initialize(ctx: Context<Initialize>) -> Result<()> {
        ...
        // Hard-coded gatekeeper reference (optional)
        let gatekeeper = Pubkey::from_str("uniqobk8oGh4XBLMqM68K8M2zNu3CdYX7q5go7whQiv")
            .map_err(|_| error!(ErrorCode::InvalidPubkey))?;
        data.gatekeeper_network = gatekeeper;

        // Set up the daily amount, mint, etc.
        data.mint = ctx.accounts.mint.key();
        data.daily_amount = 1440;
        data.last_claim_timestamp = 0;

        // The authority is the first "owner"
    ...
    /// (2) Claim tokens with a daily logic. 
    /// If you only want owners to claim, call `is_authorized` in this function.
    pub fn claim(ctx: Context<Claim>) -> Result<()> {
        ...
        if minted_amount > 0 {
            ...
            // Mint to user's associated token account
            token::mint_to(
...
pub struct Initialize<'info> {
    #[account(init, payer = authority, space = Airdrop::SIZE)]
...
// Optional: Only let owners claim
